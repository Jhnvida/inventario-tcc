  ...
  try {
    const resultado = await db.query(`
      SELECT
        ...
        COALESCE(SUM(estoque * preco), 0) AS valor_total,
        COUNT(*) FILTER (WHERE estoque <= minimo) AS em_alerta,
        (SELECT COUNT(*) FROM pedidos WHERE status IN ('aberto', 'parcial')) AS pedidos_abertos
      FROM produtos
    `);
    ...
    res.status(500).json({ erro: "Erro ao carregar dashboard", detalhe: erro.message });
  }
}
